...
export class SoundpadClient {
  ...
  async setPlayMode(mode: number): Promise<SoundpadResponse> {
    try {
  ...
  async setSpeakersOnly(enabled: boolean): Promise<SoundpadResponse> {
    try {
  ...
  async restartSoundpad(): Promise<SoundpadResponse> {
    return new Promise((resolve) => {
      // Kill Soundpad using taskkill (works regardless of pipe command support)
      const killer = spawn('taskkill', ['/F', '/IM', 'Soundpad.exe'], { stdio: 'ignore' });
      ...
      killer.on('close', (code) => {
        ...
        setTimeout(() => {
          ...
          const proc = spawn(soundpadPath, [], { detached: true, stdio: 'ignore' });
          proc.unref();
